...
public class CommandManagerV1_15_2 implements CommandManager {
    ...
    public CommandBuilder createCommand(String name) {
        return new LiteralCommandBuilderV1_15_2(name.toLowerCase());
    }

    @Override
    public CommandBuilder createCommand(String name, ArgumentType argumentType) {
        return new ArgumentBuilderV1_15_2(name.toLowerCase(), (ArgumentTypeV1_15_2) argumentType);
    }
    ...
    public Command register(String prefix, CommandBuilder builder) {
        ...
        CommandNode<CommandListenerWrapper> commandNode = ((CommandBuilderV1_15_2<?>) builder).toBrigadier().build();
        if (builder instanceof LiteralCommandBuilderV1_15_2) {
        ...
        Command command = new CommandV1_15_2(commandNode);
    ...
    public void onLoad(ServerLoadEvent event) {
        ...
        for (String prefix : REGISTERED.keySet()) {
            for (Command command : REGISTERED.get(prefix)) {
                CommandNode<CommandListenerWrapper> commandNode = ((CommandV1_15_2) command).toBrigadier();
