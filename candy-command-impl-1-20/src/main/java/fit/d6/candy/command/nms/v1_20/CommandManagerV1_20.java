...
public class CommandManagerV1_20 implements CommandManager {
    ...
    public CommandBuilder createCommand(String name) {
        return new LiteralCommandBuilderV1_20(name.toLowerCase());
    }

    @Override
    public CommandBuilder createCommand(String name, ArgumentType argumentType) {
        return new ArgumentBuilderV1_20(name.toLowerCase(), (ArgumentTypeV1_20) argumentType);
    }
    ...
    public Command register(String prefix, CommandBuilder builder) {
        ...
        CommandNode<CommandSourceStack> commandNode = ((CommandBuilderV1_20<?>) builder).toBrigadier().build();
        if (builder instanceof LiteralCommandBuilderV1_20) {
        ...
        Command command = new CommandV1_20(commandNode);
    ...
    public void onLoad(ServerLoadEvent event) {
        ...
        for (String prefix : REGISTERED.keySet()) {
            for (Command command : REGISTERED.get(prefix)) {
                CommandNode<CommandSourceStack> commandNode = ((CommandV1_20) command).toBrigadier();
