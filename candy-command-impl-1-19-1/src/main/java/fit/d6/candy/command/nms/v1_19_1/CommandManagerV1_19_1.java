...
public class CommandManagerV1_19_1 implements CommandManager {
    ...
    public CommandBuilder createCommand(String name) {
        return new LiteralCommandBuilderV1_19_1(name.toLowerCase());
    }

    @Override
    public CommandBuilder createCommand(String name, ArgumentType argumentType) {
        return new ArgumentBuilderV1_19_1(name.toLowerCase(), (ArgumentTypeV1_19_1) argumentType);
    }
    ...
    public Command register(String prefix, CommandBuilder builder) {
        ...
        CommandNode<CommandSourceStack> commandNode = ((CommandBuilderV1_19_1<?>) builder).toBrigadier().build();
        if (builder instanceof LiteralCommandBuilderV1_19_1) {
        ...
        Command command = new CommandV1_19_1(commandNode);
    ...
    public void onLoad(ServerLoadEvent event) {
        ...
        for (String prefix : REGISTERED.keySet()) {
            for (Command command : REGISTERED.get(prefix)) {
                CommandNode<CommandSourceStack> commandNode = ((CommandV1_19_1) command).toBrigadier();
