...
public class CommandManagerV1_18 implements CommandManager {
    ...
    public CommandBuilder createCommand(String name) {
        return new LiteralCommandBuilderV1_18(name.toLowerCase());
    }

    @Override
    public CommandBuilder createCommand(String name, ArgumentType argumentType) {
        return new ArgumentBuilderV1_18(name.toLowerCase(), (ArgumentTypeV1_18) argumentType);
    }
    ...
    public Command register(String prefix, CommandBuilder builder) {
        ...
        CommandNode<CommandSourceStack> commandNode = ((CommandBuilderV1_18<?>) builder).toBrigadier().build();
        if (builder instanceof LiteralCommandBuilderV1_18) {
        ...
        Command command = new CommandV1_18(commandNode);
    ...
    public void onLoad(ServerLoadEvent event) {
        ...
        for (String prefix : REGISTERED.keySet()) {
            for (Command command : REGISTERED.get(prefix)) {
                CommandNode<CommandSourceStack> commandNode = ((CommandV1_18) command).toBrigadier();
